...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "MoguyCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating MoguyCode", () => {
  expect(
...
test("ignores stale background MoguyCode checks after removing a WSL server", async () => {
  persistedServers = []
...
test("ignores stale startup MoguyCode checks after removing a WSL server", async () => {
